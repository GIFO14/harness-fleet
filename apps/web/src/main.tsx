...
function App() {
  ...
  const control = async (action: string, body = {}) => { try { await api(`/fleets/${fleet.id}/${action}`, post(body)); await load(fleet.id); } catch (e) { setError((e as Error).message); } };
  return <div className="shell">
    ...
    <main>
      ...
      <><header><div><p className="eyebrow">{fleet.id}</p><h1>{fleet.spec.fleet_name}</h1><p>{fleet.spec.goal}</p></div><div className="actions">
        {fleet.status === "waiting_for_confirmation" && <><button onClick={() => setEditor(JSON.stringify(fleet.spec, null, 2))}>Edit plan</button><button className="launch" onClick={() => { const asksFull = [fleet.spec.orchestrator, ...fleet.spec.workers].some((x: any) => x.permission_profile === "full-access"); const fullAccessConfirm = asksFull ? window.confirm("This plan requests full access. Authorize unsandboxed execution?") : false; if (!asksFull || fullAccessConfirm) void control("launch", { confirm: true, fullAccessConfirm }); }}>Confirm & launch</button></>}
        {fleet.status === "running" && <button onClick={() => control("pause")}>Pause</button>}{["paused", "needs_attention", "paused_orchestrator_unavailable"].includes(fleet.status) && <button onClick={() => control("resume")}>Resume</button>}
        {fleet.status === "paused_orchestrator_unavailable" && <button onClick={async () => { const replacement = window.prompt("Replacement harness: pi, claude-code, or codex", fleet.orchestrator?.harness ?? "codex"); if (!replacement) return; try { await api(`/fleets/${fleet.id}/orchestrator`, { method: "PUT", body: JSON.stringify({ harness: replacement }) }); await load(fleet.id); } catch (e) { setError((e as Error).message); } }}>Replace orchestrator</button>}
        <button className="danger" onClick={() => control("kill", {})}>Kill all</button></div></header>
    ...
    {drawer && <div className="drawer"><button className="close" onClick={() => setDrawer(undefined)}>×</button><p className="eyebrow">Worker</p><h2>{drawer.nodeId}</h2>
      ...
      <div className="drawer-actions"><button onClick={() => control(`relaunch/${drawer.nodeId}`)}>Relaunch</button><button className="danger" onClick={() => control("kill", { nodeId: drawer.nodeId })}>Kill</button></div>
    </div>}
